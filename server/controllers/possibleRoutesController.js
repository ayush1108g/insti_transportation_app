...
exports.getPossibleRoutes = catchAsync( async (req, res) => {
    ...
    for (const schedule of schedules) {
      ...
      const startLocationIndex = busStops.findIndex(stop => String(stop.stopId) === String(startLocationId));
      const endLocationIndex = busStops.findIndex(stop => String(stop.stopId) === String(endLocationId));

    
      if (startLocationIndex !== -1 && endLocationIndex !== -1) {
        if (busStops[startLocationIndex].stopNumber < busStops[endLocationIndex].stopNumber) {
